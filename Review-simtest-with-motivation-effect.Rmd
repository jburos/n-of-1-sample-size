...
simd %>%
  ...
  dplyr::distinct(duration_group, notify_moderate, motivation_high, scales::percent(invlogit_linpred))  %>%
  kableExtra::kable()
```
...
simfits %>%
  ...
  dplyr::filter(parname %in% c('b_duration_27_days', 'b_duration_5_days', 'b_notify_moderate', 'b_motivation')) %>%
